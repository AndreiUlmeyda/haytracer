...
spec = modifyMaxSuccess (const 1000) $ do
  describe "three tuples" $ do
    let point = MkPoint $ MkThreeTuple {x = 4.3, y = -4.2, z = 3.1}
    let vector = MkVector $ MkThreeTuple {x = 4.3, y = -4.2, z = 3.1}
    let zeroPoint = MkPoint $ MkThreeTuple {x = 0, y = 0, z = 0}
    let zeroVector = MkVector $ MkThreeTuple {x = 0, y = 0, z = 0}
    ...
    it "addition" $ do
      ...
      addPoints point point `shouldBe` Right "adding a point to a point does not have meaning in this context"
